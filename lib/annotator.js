...
class AnnotationCreator {
    ...
    for (const module of modules) {
      ...
      for (let i = 0; i < goModLines.length; i++) {
        if (goModLines[i].includes(module)) {
          ...
          const filePath =
            workingDirectory === '.' ? 'go.mod' : path.join(workingDirectory, 'go.mod');
  ...
  annotateCallSites(callSites, workingDirectory) {
    ...
    for (const site of Object.values(groupedSites)) {
      ...
      const filePath =
        workingDirectory === '.' ? site.filename : path.join(workingDirectory, site.filename);
